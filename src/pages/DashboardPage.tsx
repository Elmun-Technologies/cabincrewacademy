...
import {
  ...
  Sparkles, ArrowRight, Target, Award, Zap, ChevronRight,
  Star, BookOpen, ClipboardCheck, Rocket, Compass, Gauge as GaugeIcon,
  MapPin, Clock3, Wind,
} from 'lucide-react'
...
// ─── Stat tile ───────────────────────────────────────────────────
function StatTile({
  icon, label, value, suffix = '', gradient, delay = '',
}: { icon: React.ReactNode; label: string; value: number; suffix?: string; gradient: string; delay?: string }) {
  return (
    <div className={cn('slide-in-up rounded-2xl p-4 text-white shadow-md hover-lift', gradient, delay)}>
...
export function DashboardPage() {
  ...
  // Days to ready estimate (very rough: needs ~85 readiness; ~3 readiness/day average)
  const remainingReadiness = Math.max(0, 85 - readiness.overall)
  const daysToReady = Math.ceil(remainingReadiness / 3)

  // Recent badges + locked previews
  ...
  return (
    <div className="space-y-6 pb-4">
      ...
      <section className="slide-in-up gradient-cockpit relative overflow-hidden rounded-3xl p-5 text-white shadow-xl sm:p-6">
        ...
        <div className="relative">
          <div className="flex items-center gap-3">
            <div className="badge-glow flex h-12 w-12 items-center justify-center rounded-full bg-gradient-to-br from-etihad-gold to-orange-500 text-base font-extrabold text-white shadow-lg">
              {initials}
            </div>
            <div>
      ...
      <section className="slide-in-up delay-2">
        <div className="mb-3 flex items-center justify-between">
          <h2 className="flex items-center gap-2 text-sm font-bold uppercase tracking-wider text-etihad-dark">
            <Target className="h-4 w-4 text-etihad-gold" />
            {t('dashboard.missionBriefing')}
          </h2>
          <Link to="/daily" className="flex items-center gap-1 text-xs font-semibold text-etihad-blue hover:underline">
          ...
          <div className="relative">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {nextLesson ? (
                ...
                <Link to="/assessment">
                  <Button variant="gold" size="sm" className="shadow-lg pulse-ring">
              ...
              <Link to="/daily">
                <Button size="sm" className="border border-white/30 bg-white/10 text-white hover:bg-white/20 backdrop-blur">
      ...
      {/* ─── Cockpit Instruments (Readiness gauges) ─────────── */}
      <section className="slide-in-up delay-3">
        ...
        <div className="gradient-cockpit relative overflow-hidden rounded-3xl p-5 shadow-xl">
          ...
          <div className="relative grid grid-cols-4 gap-2 sm:gap-4">
            <Gauge value={readiness.english} label={t('readiness.english')} color="#60a5fa" icon={<Languages className="h-3.5 w-3.5" />} />
            <Gauge value={readiness.aviation} label={t('readiness.aviation')} color="#bd8b13" icon={<Plane className="h-3.5 w-3.5" />} />
            <Gauge value={readiness.softSkills} label={t('readiness.softSkills')} color="#a78bfa" icon={<Sparkles className="h-3.5 w-3.5" />} />
            <Gauge value={readiness.documents} label={t('readiness.documents')} color="#34d399" icon={<FileText className="h-3.5 w-3.5" />} />
          </div>
      ...
      <section className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <StatTile
          ...
          value={xp.currentStreak}
          suffix={xp.currentStreak > 0 ? '🔥' : ''}
        ...
        <section className="slide-in-up delay-3">
          <Card className="overflow-hidden border-0 p-0 shadow-md">
            <div className="flex items-center justify-between border-b border-gray-100 bg-gradient-to-r from-etihad-light to-white p-4">
              <div>
                <p className="text-[10px] font-bold uppercase tracking-wider text-etihad-gold">
              ...
              <div className="text-right">
                <div className="text-2xl font-extrabold text-etihad-blue">
                  <AnimatedNumber value={phaseProgressValue} />%
                </div>
      ...
      {/* ─── Quick Actions ──────────────────────────────────── */}
      <section>
        <h2 className="mb-3 flex items-center gap-2 text-sm font-bold uppercase tracking-wider text-etihad-dark">
          <Wind className="h-4 w-4 text-etihad-blue" />
      ...
      {/* ─── Two-column row: Week + Quote ───────────────────── */}
      <section className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Card className="slide-in-up delay-2">
          <h3 className="mb-3 flex items-center gap-2 text-sm font-bold uppercase tracking-wider text-etihad-dark">
            <Calendar className="h-4 w-4 text-etihad-gold" />
            {t('dashboard.weekActivity')}
          </h3>
        ...
        <Card className="slide-in-up delay-3 relative overflow-hidden gradient-etihad text-white">
          ...
          <div className="relative">
            ...
            <p className="mt-3 text-base font-semibold italic leading-relaxed sm:text-lg">
              {t(quoteKey)}
            </p>
      ...
      <section className="slide-in-up delay-4">
        <div className="mb-3 flex items-center justify-between">
          <h2 className="flex items-center gap-2 text-sm font-bold uppercase tracking-wider text-etihad-dark">
            <Award className="h-4 w-4 text-etihad-gold" />
            {t('dashboard.achievements')}
          </h2>
          <Link to="/profile" className="flex items-center gap-1 text-xs font-semibold text-etihad-blue hover:underline">
            {t('dashboard.viewAll')} <ChevronRight className="h-3 w-3" />
          </Link>
        </div>
          ...
          <div className="flex gap-3 overflow-x-auto pb-2">
            {badges.map((b, i) => (
              <div
                ...
                className="scale-in flex min-w-[110px] flex-col items-center rounded-2xl bg-gradient-to-br from-etihad-gold/15 to-yellow-100 p-3 shadow-sm card-glow"
              >
      ...
      {readiness.overall >= 85 && (
        <section className="slide-in-up delay-5">
          <div className="gradient-emerald relative overflow-hidden rounded-3xl p-5 text-center text-white shadow-xl">
            ...
            <div className="relative">
              <div className="mx-auto mb-2 flex h-14 w-14 items-center justify-center rounded-full bg-white/20 backdrop-blur pulse-ring">
